...
public class ManagerCookImpl extends Staff {
    public ManagerCookImpl(double salary, Position position, String name) {
        super(salary, position, name);
    }

    public Staff hireStaff(CookImpl entity){
        return new Executor().insertStaffInfo(entity, new Executor().connectToEmployees());
    }

    public Staff fireStaff(CookImpl entity){
        return new Executor().deleteStaffInfo(entity, new Executor().connectToEmployees());
    }

    public Staff promoteEntity(@NotNull CookImpl entity, Position position, double salary){
        return new Executor().updateStaffByNameAndPromote(entity.getName(), position, salary, new Executor().connectToEmployees());
    }

    public Staff changeSalary(@NotNull CookImpl entity, double salary){
